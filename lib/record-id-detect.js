...
const { AbsoluteV5ApiUrl } = require('./absolute-v5-api-url')
...
RecordId.detect = function (recordIdString) {
  ...
  if (recordIdString) {
    ...
    if (firstCodePoint === CODE_POINT_PERIOD) {
      ...
      if (trimmedRecordIdString.indexOf('/v5/') !== -1) {
        kind = AbsoluteV5ApiUrl
      } else if (trimmedRecordIdString.indexOf('/v4/') !== -1) {
        kind = AbsoluteV4ApiUrl
      }
    } else if (trimmedRecordIdString.startsWith('/v5/')) {
      kind = RelativeV5ApiUrl
